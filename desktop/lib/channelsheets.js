...
function save(dir, channel, data = {}) {
  ...
  const next = {
    ...
    anchorImage: data.anchorImage != null ? String(data.anchorImage).slice(0, 300) : (cur.anchorImage || ''),
...
function draftPrompt(brand = {}, channel = '', channelName = '', platformDir = '') {
  ...
  return (
    ...
    `- 각 시트는 250~500자. 추상어는 구체 사물·제스처로 환원.\n` +
    `- 출력은 JSON 하나만: {"master":"...","character":"..."} (코드펜스 금지)`
  );
}

// AI 초안 응답 파싱 — {master, character} 추출. 실패 시 null.
function parseDraft(out) {
  ...
  if (!master && !character) return null;
  return { master: master.slice(0, 4000), character: character.slice(0, 4000) };
}
...
function compileDirective(dir, channel) {
  ...
  return `\n[채널 시트 락인 — ${channel} · 반드시 준수]\n`
    + `이 채널의 모든 이미지는 아래 마스터/캐릭터 시트를 일관되게 따른다(팔레트·조명·스타일·등장 주체를 프레임 간 고정). `
    + `VISUAL DIRECTION·플랫폼 방향보다 이 락인 시트가 우선한다.\n`
    + parts.join('\n\n') + '\n\n';
}

module.exports = { get, save, setLock, list, compileDirective, draftPrompt, parseDraft, sheetsDir };
