...
function draftPrompt(brand = {}, channel = '', channelName = '', platformDir = '') {
  ...
  return (
    ...
    `브랜드 팔레트: ${palette}\n\n` +
    `[작성 규칙 — 마크다운 구조로 상세히]\n` +
    `- 세 필드(master, character, guidelines)를 각각 마크다운(## 소제목 + 불릿)으로 구조화해 상세히 작성한다. 아래 섹션 골격을 채우되 이 채널에 무의미한 섹션은 생략하고, 필요하면 섹션을 추가한다.\n` +
    ...
    `[character 섹션 골격 — 캐릭터 모델 시트: 반복 주체를 매 컷 동일하게 재현할 "정체의 규칙"]\n` +
    ...
    `  · 사람이면 natural skin texture, visible pores 명시. 반복 주체가 없어야 하는 채널이면 "반복 주체 없음 — 오브젝트/손 중심" 원칙과 고정할 오브젝트/공간·시그니처 디테일을 적는다.\n\n` +
    ...
    `- 분량: master·character는 각 500~900자, guidelines는 800~1500자(전략별 세부까지 채운다).\n` +
    `- 출력은 JSON 하나만: {"master":"...","character":"...","guidelines":"..."} — 각 값은 소제목·불릿·줄바꿈을 포함한 마크다운 문자열이다. 코드펜스 금지.`
  );
}
