...
| 선거·정치 맥락의 실존 인물 AI 합성(유명 여부 무관) 또는 선거운동성 콘텐츠 | 공직선거법의 딥페이크 선거운동 제한 대상 — SMB 상업 계정에서 다룰 사유가 없음 | BLOCK |
| AI 표시 의무가 없는 단순 그래픽(스탯 카드 등) | 표시 권장 (안전판) | PASS + 권장 메모 |

AI 표시는 의무가 아닌 경우에도 표시하면 추후 리스크가 줄어듭니다 — 결과 파일에 권장 사항으로 남깁니다.

**단계적 상향 (AI기본법 대비):** 1단계(현행) — 카피라이터 에이전트가 AI 생성 비주얼 예정 포스트의 해시태그에 `#AI생성`을 기본 삽입하므로, 이 게이트의 고지 누락 WARN은 삽입 실패 신호입니다. 2단계 — AI기본법 시행령이 확정되어 표시 의무 발효일이 정해지면 첫 행의 WARN을 **발효일 조건부 BLOCK**으로 상향합니다. 시행령 미확정 상태의 일괄 BLOCK은 과차단 리스크가 있어 적용하지 않습니다.
...
**결정적 검증 스크립트 (있으면 필수 사용):** 파일명 린트(5.1), 계약 필드 존재(5.2), 글자수 재계산(5.3)은 눈대중으로 세지 말고 스크립트를 실행해 그 출력을 판정 근거로 사용합니다:

```bash
python3 skills/kr-guardrail-check/scripts/verify_contracts.py --workroot .        # 사람이 읽는 표
python3 skills/kr-guardrail-check/scripts/verify_contracts.py --workroot . --json # 기계 파싱용
```

스크립트의 BLOCK/WARN 행을 판정표에 그대로 옮기고, 스크립트가 다루지 않는 판단(기대 레인 파일 **누락** — workflow-status 기준, `Sponsored: Yes` 고지 문구의 **위치**, 캘린더 대응 5.4)만 직접 수행합니다. 파이썬이 없는 환경에서만 아래 규칙대로 수동 검증으로 대체하고, 그 사실을 리포트 `Mode:`에 명시합니다.
...
필드명 매칭은 **대소문자를 무시**합니다 — `Visual direction:`처럼 표기만 다른 필드는 존재하는 것으로 인정하되, 표준 표기(대문자)로의 정정을 권고 사항으로 남깁니다. 철자 자체가 다르거나 번역된 변형(`VISUAL DIR:`, `비주얼 디렉션:` 등)만 누락으로 처리합니다.
...
- **글자수는 스크립트가 셉니다** — LLM의 글자수 계산은 재현이 안 됩니다. `scripts/verify_contracts.py`가 X CJK 가중(2유닛)·URL 23유닛·Threads 500자를 결정적으로 재계산합니다. 스크립트 출력과 작성 스킬의 표기가 다르면 스크립트가 맞습니다.
